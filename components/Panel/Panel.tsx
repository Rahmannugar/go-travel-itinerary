...
const Panel = ({ isOpen, onClose }: PanelProps) => {
  ...
  const renderPanelItem = (item: (typeof panelItems)[0], index: number) => {
    ...
    const baseClasses =
      "flex items-center gap-3 cursor-pointer mb-6 md:mb-10 hover:scale-105 transition-transform duration-200 ease-in-out";
    ...
    if (isRouteable) {
      return (
        ...
        >
          {item.icon}
          <span className="text-sm md:hidden lg:block text-[#647995]">
            {item.name}
          </span>
        </Link>
      );
  ...
  return (
    <aside
      className={`
        fixed top-0 left-0 w-full h-[100dvh]
        bg-white
        transition-all duration-300 ease-in-out z-30
        md:sticky md:top-24 md:h-[calc(100vh-6rem)]
        md:w-[300px] xl:w-[350px]
        ${
          isOpen
            ? "translate-x-0 pt-20 md:pt-0"
            : "-translate-x-full pt-0 md:translate-x-0"
        }
      `}
    >
      <div className="h-full flex flex-col">
        <div className="flex-1 overflow-y-auto scrollbar-hide p-10 md:flex md:flex-col md:justify-center md:items-center lg:block">
          {panelItems.map((item, index) => renderPanelItem(item, index))}
        </div>

        <div className="bg-background h-[86px] hidden lg:flex px-3 mx-6 mb-6 items-center gap-2 justify-between">
